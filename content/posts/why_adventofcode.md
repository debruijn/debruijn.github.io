...
[Advent of Code](www.adventofcode.com) is coming up again, starting December 1st. It is a yearly event with programming
...
the current but also for the past 9 years.

### What is Advent of Code?

### Get started with programming - or get back to it
First of all, the event can be a great way to learn programming, or getting back to it. If you are 100% new to
...
language on your own, then that is totally fine, but also something like Advent of Code might not be the thing for you -
at least not now. Find someone to help you get started, and then you can always come back to this later.
...
I have done something similar recently with a similar coding event called [coding quest](www.codingquest.io), for which
I have done the first series of practice challenges all in R, a language I used to work with a lot but less and less
over time. I could notice a big difference between the first and last challenge in remembering the right way to do
something in R, so for sure if I know that an R project is coming up, I will do something similar again.
...
- Object-oriented programming (if the daily challenge was suitable for it)
- Pure python without dependencies - so stay away from numpy and pandas which I would tend towards
...
I currently can't imagine not reaching to those solutions first (when suitable), and it is because I have build up that
habit over time doing all those Advent of Code challenges. Of course, for specific use cases using packages makes a lot
of sense, but having the urge to import numpy or pandas just to sum a list of numbers was not the right habit.
...
to get into it in work. In the last year, I have done so, and I have [blogged about](/posts/rust-python-01) before.
...
worthwhile in your daily life at some point.
...
- .. and then: have a look at the advent of code [subreddit](www.reddit.com/r/adventofcode) for how other people have
solved the problem.
...
- I can also highly recommend [Everybody Codes](www.everybody.codes) which had its first event this November 2024 with
...
- There is also [Coding Quest](www.codingquest.io), for which I have only done the introductory set of puzzles so far
but I plan to come back to this in the new year with new languages to learn.
- This December 2024 will also kick off [Advent of SQL](www.adventofsql.com), which will also introduce daily puzzles
